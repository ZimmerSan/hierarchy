...
<div class="container">
                ...
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">
                            <spring:message code="page.matrix"/>
                        </li>
                    </ol>
                </nav>
            </div>
    ...
    <h1 class="demo-panel-title"><spring:message code="problem.matrix"/></h1>
    <div class="row">
        <div class="col">
                        ...
                        <table class="matrix">
                            <tr>
                                ...
                                <c:forEach items="${problem.factors}" var="factor" varStatus="status">
                                    <td data-toggle="tooltip" data-placement="top" title="${factor.name}">${factor.code}</td>
                                </c:forEach>
                            </tr>
                            <c:forEach items="${problem.matrix}" var="row" varStatus="y_status">
                                <tr>
                                    <td data-toggle="tooltip" data-placement="left" title="${problem.factors.get(y_status.index).name}">${problem.factors.get(y_status.index).code}</td>
                                    <c:forEach items="${problem.matrix[y_status.index]}" var="cell"
                                               varStatus="x_status">
                                        <c:if test="${x_status.index > y_status.index}">
                                            <td><form:input path="matrix[${y_status.index}]" value="${cell}"
